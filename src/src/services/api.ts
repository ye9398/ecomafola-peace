...
// Production: Use environment variable VITE_API_BASE_URL or default to empty (use mock data)
...
async function request(path: string, opts: RequestInit = {}) {
  ...
  // If no BASE URL configured, use mock data immediately
  if (!BASE || BASE.trim() === '') {
    console.log('[API] No backend configured, using mock data')
  ...
  try {
    ...
    if (!contentType.includes('application/json')) {
      console.warn(`[API] Received non-JSON response for ${path}, using mock data`)
