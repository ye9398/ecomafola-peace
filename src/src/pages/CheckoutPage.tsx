...
export default function CheckoutPage() {
  ...
  const shippingItems = items.map(i => ({ product_id: i.id, quantity: i.quantity }))
                  ...
                  {items.map(item => (
                    <div key={item.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-sans font-medium text-gray-800 truncate">{item.name}</p>
                        <p className="text-xs font-sans text-gray-500">${item.price_usd.toFixed(2)} each</p>
                      </div>
              ...
              <div className="space-y-3 mb-4">
                {items.map(item => (
                  <div key={item.id} className="flex justify-between text-sm font-sans">
                    ...
                    <span className="text-gray-800 font-medium">${(item.price_usd * item.quantity).toFixed(2)}</span>
                  </div>
                ))}
              </div>
