...
function areFilesMatching(fileName, filePath, matcher) {
...
🚨 ${ansi.red("[vite-plugin-image-optimizer]")} - errors during optimization: `, ansiColors));
  ...
  errorsMap.forEach((message, name) => {
    logger.error(
      decideStyle(
        `${ansi.dim(pathe.basename(outDir))}/${ansi.blueBright(name)}${" ".repeat(2 + maxKeyLength - name.length)} ${ansi.red(message)}`,
        ansiColors
      )
    );
  });
...
✨ ${ansi.cyan("[vite-plugin-image-optimizer]")} - optimized images successfully: `, ansiColors));
  ...
  sizesMap.forEach((value, name) => {
    ...
    logger.info(
      decideStyle(
        ansi.dim(pathe.basename(outDir)) + "/" + ansi.blueBright(name) + " ".repeat(2 + maxKeyLength - name.length) + ansi.gray(`${percentChange} ${" ".repeat(valueKeyLength - `${ratio}`.length)}`) + " " + sizeText,
        ansiColors
      )
    );
    if (!skipWrite) {
      ...
      if (publicDir && options.includePublic) {
        ...
        if (files.length > 0) {
          ...
          const handles = files.map(async (publicFilePath) => {
            ...
            const fullFilePath = pathe.resolve(rootConfig.root, outputPath, filePath);
